...
//	{
//		vec3 sample = vec3(view * vec4(AO_TBN * (samples[i]), 0.0));
...
//	FragColor = occlusion;
	...
	vec3 fragPos   = (texture(gPosition, TexCoords)).xyz;//frag pos in view space
	vec3 normal    = (texture(gNormal, TexCoords)).rgb;//model normal in view space
	vec3 randomVec = (texture(texNoise, TexCoords * noiseScale)).xyz;
