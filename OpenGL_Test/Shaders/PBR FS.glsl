...
{		
    ...
    for(int i = 0; i < NUM_POINT_LIGHTS; ++i) 
    {
        ...
        vec3 L = normalize(vec3(pointLights[i].position) - WorldPos);
        vec3 H = normalize(V + L);
        float distance = length(vec3(pointLights[i].position) - WorldPos);
        float attenuation = 1.0 / (distance * distance);
        vec3 radiance = vec3(pointLights[i].diffuse) * attenuation;
